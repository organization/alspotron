    ...
    <div class={'flex-1 flex flex-col justify-start items-stretch gap-1 p-4 fluent-scrollbar'}>
      ...
      <For each={availableGameList()}>
        {(game) => (
          ...
          >
            ...
            <button class={'btn-error h-full'} onClick={() => onRemoveGame(game.path)}>
      ...
      >
        ...
        <For each={config()?.views}>
          {(view) => (
            <Card class={'flex flex-row justify-start items-center gap-4'} onClick={() => onAddGame(view.name)}>
              <div class={'w-6 h-6'}/>
              <div class={'text-md'}>
                {view.name}
              </div>
      ...
      >
        ...
        <For each={config()?.views}>
          {(view) => (
            <Card class={'flex flex-row justify-start items-center gap-4'} onClick={() => onApplyTheme(view.name)}>
              <div class={'w-6 h-6'}/>
              <div class={'text-md'}>
                {view.name}
              </div>
