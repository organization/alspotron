  ...
  override getDisplayBounds() {
    if (this.nodeWindowManager) {
      ...
      const window = windowManager
        .getWindows()
        .find((window) => window.processId === this.pid);
      ...
      if (typeof windowBounds?.width === 'number')
        bounds.width = windowBounds.width - 16; // MAGIC VALUE
      if (typeof windowBounds?.height === 'number')
        bounds.height = windowBounds.height - 39; // MAGIC VALUE

      const isFullscreen =
        (windowBounds?.width ?? 0) <= 10 ||
        (windowBounds?.height ?? 0) <= 10 ||
